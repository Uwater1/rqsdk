...
# 0.0002 commission per leg (approx 2 RMB per contract of 10000 shares)
...
def load_date_folder(date_dir: str) -> dict:
    ...
    """
    folders = ['510050XSHG', '510300XSHG', '510500XSHG']
    ...
    for sub in folders:
        ...
        for fpath in sorted(glob.glob(os.path.join(sub_dir, '*.parquet'))):
            ...
            inst = rqdatac.instruments(oid)
            if inst is None or inst.type != 'Option':
                continue
            prefix = inst.underlying_symbol
            expiry_str = inst.maturity_date
...
def build_unified_timeline(group: dict, freq='2S'):
    ...
    return all_times, c_ask, c_bid, p_ask, p_bid, np.array(strikes, dtype=np.float64)
...
def find_best_boxes(c_ask, c_bid, p_ask, p_bid, ks, dte):
    ...
    short_K1      = np.zeros(T, np.float64)
    short_K2      = np.zeros(T, np.float64)
...
def batch_by_minute(timestamps, kernel_out, prefix, expiry, dte):
    ...
    for mn in unique_mins:
        ...
        if lr.max() > 0:
            ...
            t = idxs[best]
            row.update(long_K1=float(lK1[t]), long_K2=float(lK2[t]),
        ...
        if sr.max() > 0:
            ...
            idxs = np.where(mask)[0]
            t = idxs[best]
            row.update(short_K1=float(sK1[t]), short_K2=float(sK2[t]),
...
def run_scanner(date_dir: str, out_dir: str = '.'):
    ...
    # Each stores up to 3 best rows seen across all groups for that category
    near_best:  dict = defaultdict(list)   # long box, near DTE
    mid_best:   dict = defaultdict(list)   # long box, mid DTE
    short_best: dict = defaultdict(list)   # short box, any DTE
    ...
    for (prefix, expiry), g in groups.items():
        ...
        for r in rows:
            ...
            if r['long_ret'] > 0:
                if dte == min_dte:
                    near_best[mn].append(r)
                    near_best[mn].sort(key=lambda x: x['long_ann'], reverse=True)
                    near_best[mn] = near_best[mn][:3]
                elif mid_dte is not None and mid_dte <= 60 and dte == mid_dte:
                    mid_best[mn].append(r)
                    mid_best[mn].sort(key=lambda x: x['long_ann'], reverse=True)
                    mid_best[mn] = mid_best[mn][:3]

            # ── short box (any DTE ≤60) ──
            if r['short_ret'] > 0 and dte <= 60:
                short_best[mn].append(r)
                short_best[mn].sort(key=lambda x: x['short_ann'], reverse=True)
                short_best[mn] = short_best[mn][:3]
...
def write_csvs(near_best, mid_best, short_best, date_str, out_dir):
    ...
    def to_df(best_dict, row_fn, header, empty_row):
        rows = []
        for mn in all_mins:
            if mn in best_dict and best_dict[mn]:
                for r in best_dict[mn]:
                    rows.append(row_fn(mn, r))
            else:
    ...
    paths = {
        'near': os.path.join(out_dir, f'near-long-{date_str}-etf.csv'),
        'mid':  os.path.join(out_dir, f'mid-long-{date_str}-etf.csv'),
        'short':os.path.join(out_dir, f'short-{date_str}-etf.csv'),
    }
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python boxx_etf.py <data-deep/YYYY-MM-DD> [output_dir]")
        sys.exit(1)
    date_dir = sys.argv[1]
