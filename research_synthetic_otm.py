...
def analyze_synthetic_otm(years=None):
    ...
    for option_type in ["C", "P"]:
        ...
        # metrics: [num_levels (6), metrics (5)] -> [count, sum_pnl, sum_wins, sum_total_wins, min_pnl]
        metrics = calculate_research_metrics_numba(
            strikes, s0s, ret_val, prices, worthless, boundaries, is_call
        )
        ...
        for lv in range(6):
            ...
            pnl_sum = metrics[lv, 1]
            wins = metrics[lv, 2]
            total_wins = metrics[lv, 3]
            min_pnl = metrics[lv, 4]
