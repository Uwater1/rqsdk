from live_boxx_far import THRESHOLD
import sys
import time
import threading
import pytz
        ...
        for prefix, expiries in options_map.items():
            for expiry, strikes_dict in expiries.items():
                ...
                if idx_i >= 0 and ann > THRESHOLD:
                    print(f"[{datetime.now(TZ).strftime('%H:%M:%S')}] Long Box Near [{prefix}]: DTE={dte} K1={ks[idx_i]} K2={ks[idx_j]} Cost={cost:.2f} Payout={payout:.2f} Ret={ret*100:.2f}% Ann={ann*100:.2f}%")
